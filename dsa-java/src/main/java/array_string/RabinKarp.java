...
public class RabinKarp {
    ...
    * */
    ...
    static void search(String pat,String text,int q){
        ...
        int N = text.length();
        int i,j;
        int p = 0;//hash value for a pattern
        int t = 0;//hash value for text
        int h = 1;//multiplier used in rolling hash function

        for(i = 0; i < M -1; i++){
            h = (h * d) % q;//used in later hashing operations
        }
        System.out.println(h);
        //calculate hash of a pattern and 1st window of a text
        for(i = 0; i < M; i++){
            p = (d * p + pat.charAt(i)) % q;
            t = (d * t + text.charAt(i)) % q;
            System.out.println("Pattern's hash value is "+p+" and text's hash value is "+t+" at "+i);
        }
        //after the above loop, we have the hash value of a pattern and first window using the
        //rolling hash function
        //slide the pattern over text one by one
        for(i = 0; i <= N - M; i++){
              // Check the hash values of a current window of
              // text and pattern.
            ...
            //calculate the hash value for the next window of a text and remove a leading digit, and trailing digit
            if(i < N - M){
